 ...
 */
package de.hybris.fashion.storefront.security.impl;
...
public class DefaultGuestCheckoutCartCleanStrategyTest
{
	...
	public void testForGuestInAnonymousCheckout() throws IOException, ServletException
	{
		...
		given(request.getRequestURL())
				.willReturn(new StringBuffer("https://electronics.local:9002/fashionstorefront/electronics/en/checkout/multi"));
		given(request.getSession()).willReturn(session);
	...
	public void testWhetherURLContainsCheckoutPattern() throws IOException, ServletException
	{
		...
		given(request.getRequestURL())
				.willReturn(new StringBuffer("https://electronics.local:9002/fashionstorefront/electronics/en/checkou/multi"));
		assertEquals(Boolean.FALSE, Boolean.valueOf(guestCheckoutCartCleanStrategy.checkWhetherURLContainsSkipPattern(request)));

		given(request.getRequestURL())
				.willReturn(new StringBuffer("electronics.local:9002/fashionstorefront/electronics/en/checkout/multi"));
		assertEquals(Boolean.FALSE, Boolean.valueOf(guestCheckoutCartCleanStrategy.checkWhetherURLContainsSkipPattern(request)));
	...
	{
		given(request.getRequestURL())
				.willReturn(new StringBuffer("https://electronics.local:9002/fashionstorefront/favicon"));
		assertEquals(Boolean.FALSE, Boolean.valueOf(guestCheckoutCartCleanStrategy.checkWhetherURLContainsSkipPattern(request)));

		given(request.getRequestURL()).willReturn(new StringBuffer(
				"https://electronics.local:9002/fashionstorefront/_ui/responsive/theme-alpha/images/favicon.ico"));
		assertEquals(Boolean.TRUE, Boolean.valueOf(guestCheckoutCartCleanStrategy.checkWhetherURLContainsSkipPattern(request)));

		given(request.getRequestURL())
				.willReturn(new StringBuffer("https://electronics.local:9002/fashionstorefront/favicon.ico"));
		assertEquals(Boolean.TRUE, Boolean.valueOf(guestCheckoutCartCleanStrategy.checkWhetherURLContainsSkipPattern(request)));
